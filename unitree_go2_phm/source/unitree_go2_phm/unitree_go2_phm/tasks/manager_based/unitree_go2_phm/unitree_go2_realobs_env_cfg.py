...
@configclass
class RealObsObservationsCfg:
    """Policy observations constrained to measurable channels."""
    ...
    class PolicyCfg(ObsGroup):
        ...
        # Reference locomotion policy core + measurable PHM channels
        ...
        energy_budget = ObsTerm(
            func=phm_mdp.available_voltage_budget_realobs,
            params={"cutoff_voltage": 24.5},
            noise=None,
        )
        thermal_stress = ObsTerm(
            func=phm_mdp.thermal_stress_realobs,
            params={
            ...
            noise=GaussianNoiseCfg(std=0.015),
        )
    ...
    class CriticCfg(ObsGroup):
        # Reference critic core + measurable PHM channels. No corruption.
...
@configclass
class RealObsRewardsCfg:
    """PHM-aware rewards without privileged-mechanical labels."""

    # Round-1 tuning (walking-priority): keep changes small (<=10%).
    ...
    thermal_safety = RewTerm(
        func=phm_mdp.thermal_margin_reward_realobs,
        weight=0.03,
        params={"std": 3.0, "limit_temp": 70.0, "warn_temp": 65.0, "coil_to_case_delta_c": 5.0},
    )
...
class UnitreeGo2RealObsEnvCfg(ManagerBasedRLEnvCfg):
    ...
    rewards: RealObsRewardsCfg = RealObsRewardsCfg()
    ...
    def __post_init__(self):
        ...
        # Real-observable policy: brownout gating follows measurable sensor voltage.
