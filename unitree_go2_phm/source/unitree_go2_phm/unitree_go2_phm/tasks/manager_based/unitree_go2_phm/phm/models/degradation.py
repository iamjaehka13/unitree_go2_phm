...
def update_fatigue_index(env: ManagerBasedRLEnv, dt: float, env_ids: Optional[torch.Tensor] = None):
    ...
    temperature = env.phm_state.coil_temp[env_ids]
    ...
    # 누적 피로도 업데이트
    env.phm_state.fatigue_index[env_ids] += delta_fatigue
    ...
    env.phm_state.fatigue_rate[env_ids] = total_fatigue_rate
...
def get_mechanical_health_index(env: ManagerBasedRLEnv, env_ids: Optional[torch.Tensor] = None) -> torch.Tensor:
    ...
    initial_health = env.phm_state.motor_health_capacity[env_ids]

    # 현재 SOH = 초기값 - 누적된 피로도
    current_health = initial_health - env.phm_state.fatigue_index[env_ids]
