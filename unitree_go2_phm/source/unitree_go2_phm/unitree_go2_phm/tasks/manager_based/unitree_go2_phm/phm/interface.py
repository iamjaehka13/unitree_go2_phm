...
# PHM 시스템 초기화
# =============================================================================
def init_phm_interface(env: ManagerBasedRLEnv):
    """
    [Isaac Lab 2.1] PHM 시스템 초기화.
    ...
    # 1. PHM State 생성 (Core)
    # PHMState.__init__() creates all fields (friction_power, stall_timer, etc.)
    env.phm_state = PHMState(num_envs, NUM_MOTORS, device)

    # 2. IMU Sensor Linking
    if hasattr(env.scene, "sensors") and "base_imu" in env.scene.sensors:
        env.phm_imu_sensor = env.scene.sensors["base_imu"]
        env.phm_imu_cfg = SceneEntityCfg("base_imu")
    else:
        env.phm_imu_sensor = None
        env.phm_imu_cfg = None

    # 5. Robot Config & Indexing
    robot: Articulation = env.scene["robot"]
    env.phm_robot_cfg = SceneEntityCfg("robot")

    joint_indices, _ = robot.find_joints(JOINT_NAMES)
    env.phm_joint_indices = joint_indices

    # 6. Optimization Constants
    env.phm_const_gravity = torch.tensor(GRAVITY_VAL, device=device)

    # 7. Initialize Buffers
    env.phm_long_term_buffer = LongTermHealthBuffer(
        num_envs=num_envs,
        num_joints=NUM_MOTORS,
        device=device
    )

    env.phm_short_term_buffer = ShortTermHealthBuffer(
        env=env,
        num_envs=num_envs,
        device=device,
        robot_cfg=env.phm_robot_cfg,
        contact_sensor_cfg=SceneEntityCfg("contact_forces"),
        joint_indices=env.phm_joint_indices
    )

    # 8. External Sync (User Data)
    # [Fix] Isaac Lab 표준: env.data 대신 env.extras 사용
    if "joint_temp" not in env.extras:
        env.extras["joint_temp"] = env.phm_state.coil_temp
    if hasattr(env.phm_state, "motor_case_temp") and ("joint_case_temp" not in env.extras):
        env.extras["joint_case_temp"] = env.phm_state.motor_case_temp


# =============================================================================
# 스텝별 메트릭 초기화 (Accumulator Reset)
# =============================================================================
def clear_step_metrics(env: ManagerBasedRLEnv):
    """
    [Critical Logic] RL Control Step 시작 전 호출.
    
    이전 Decimation Loop에서 누적된 에너지 및 전력 로그를 0으로 초기화합니다.
    """
    if hasattr(env.phm_state, "step_energy_log"):
        env.phm_state.step_energy_log.zero_()
        
    if hasattr(env.phm_state, "avg_power_log"):
        env.phm_state.avg_power_log.zero_()
        
    if hasattr(env.phm_state, "torque_saturation"):
        env.phm_state.torque_saturation.zero_()

    if hasattr(env.phm_state, "jitter_intensity"):
        env.phm_state.jitter_intensity.zero_()


# =============================================================================
# 센서 노이즈 갱신 (Static Noise Paradox 해결)
# =============================================================================
def refresh_phm_sensors(env: ManagerBasedRLEnv, env_ids: Optional[torch.Tensor] = None):
    """
    ...
    if hasattr(env.phm_state, "encoder_noise"):
        ...
        if env_ids is None:
            env.phm_state.encoder_noise.normal_(mean=0.0, std=noise_std_pos)
        else:
            env.phm_state.encoder_noise[env_ids] = torch.randn_like(
                env.phm_state.encoder_noise[env_ids]
            ) * noise_std_pos

    # 2. Velocity Noise Injection (D-term Noise) [NEW]
    # 속도 노이즈는 미분 특성상 위치 노이즈보다 주파수가 높고 큼 (예: 0.05 ~ 0.1 rad/s)
    if hasattr(env.phm_state, "encoder_vel_noise"):
        noise_std_vel = float(getattr(getattr(env, "cfg", None), "encoder_vel_noise_std_rads", 0.03))
        if env_ids is None:
            env.phm_state.encoder_vel_noise.normal_(mean=0.0, std=noise_std_vel)
        else:
            env.phm_state.encoder_vel_noise[env_ids] = torch.randn_like(
                env.phm_state.encoder_vel_noise[env_ids]
            ) * noise_std_vel

    # 3. Sample-and-hold measurement update (sensor latency / packet staleness model)
    if hasattr(env.phm_state, "encoder_meas_pos") and hasattr(env.phm_state, "encoder_meas_vel"):
        ids = slice(None) if env_ids is None else env_ids
        robot: Articulation = env.scene["robot"]
        joint_idx = getattr(env, "phm_joint_indices", slice(None))

        true_pos = robot.data.joint_pos[ids][:, joint_idx]
        true_vel = robot.data.joint_vel[ids][:, joint_idx]
        offset = env.phm_state.encoder_offset[ids]
        pos_noise = env.phm_state.encoder_noise[ids]
        vel_noise = env.phm_state.encoder_vel_noise[ids]
        ...
        prev_pos = env.phm_state.encoder_meas_pos[ids]
        prev_vel = env.phm_state.encoder_meas_vel[ids]
        env.phm_state.encoder_meas_pos[ids] = torch.where(hold_mask, prev_pos, proposed_pos)
        env.phm_state.encoder_meas_vel[ids] = torch.where(hold_mask, prev_vel, proposed_vel)

        if hasattr(env.phm_state, "encoder_hold_flag"):
            env.phm_state.encoder_hold_flag[ids] = hold_mask.squeeze(-1).float()


# =============================================================================
# PHM 동역학 업데이트 (Physics Step 직후 호출)
# =============================================================================
def update_phm_dynamics(env: ManagerBasedRLEnv, dt: float, env_ids: Optional[torch.Tensor] = None):
    """
    ...
    raw_torque = robot.data.applied_torque[env_ids][:, env.phm_joint_indices]
    raw_vel = robot.data.joint_vel[env_ids][:, env.phm_joint_indices]
    ...
    env.phm_state.applied_torque[env_ids] = applied_torque
    ...
    # 혹시 모를 NaN 방지
    env.phm_state.fatigue_index[env_ids] = torch.nan_to_num(
        env.phm_state.fatigue_index[env_ids], nan=0.0
    ).clamp(min=0.0, max=10.0)

    # -------------------------------------------------------------------------
    # 3. 진동 계산 (Safe Norm)
    # -------------------------------------------------------------------------
    if env.phm_imu_sensor is not None:
        ...
        g_val = env.phm_const_gravity if env.phm_const_gravity > 1e-6 else 9.81
        vibration = (torch.norm(kinematic_accel, dim=-1, keepdim=True) / g_val).squeeze(-1)
        
        env.phm_state.vibration_g[env_ids] = torch.clamp(vibration, 0.0, 100.0)

    # -------------------------------------------------------------------------
    # 4. [CRITICAL] 마찰 전력(Friction Power) 계산
    # -------------------------------------------------------------------------
    fatigue = env.phm_state.fatigue_index[env_ids] # 위에서 이미 clamp 함
    
    friction_bias_full = getattr(env.phm_state, "friction_bias", None)
    ...
    if friction_source is not None:
        try:
            ...
            if isinstance(env_ids, slice) or env_ids.max().item() < num_source_envs:
                ...
                if isinstance(env.phm_joint_indices, slice):
                    base_friction_torque = friction_source[env_ids]
                else:
                    safe_indices = [
                        i for i in env.phm_joint_indices
                        if 0 <= i < num_source_joints
                    ]

                    if len(safe_indices) == len(env.phm_joint_indices):
                        base_friction_torque = friction_source[env_ids][:, safe_indices]
        except Exception as err:
            if not _PHM_LOG_FLAGS["friction_source_read_failed"]:
                logging.warning(
                    "[PHM] Failed to read runtime friction coefficients; "
                    "falling back to nominal stiction. error=%s",
                    err,
                )
                _PHM_LOG_FLAGS["friction_source_read_failed"] = True
            base_friction_torque = None
    ...
    if base_friction_torque is None:
        if isinstance(env.phm_joint_indices, slice):
            ...
            target_dim = len(env.phm_joint_indices)
    ...
    # [Fix] base_friction_torque를 PHMState에 캐싱 (SSOT: env.py에서도 동일 소스 참조)
    if hasattr(env.phm_state, "base_friction_torque"):
        env.phm_state.base_friction_torque[env_ids] = base_friction_torque
    ...
    env.phm_state.friction_power[env_ids] = real_friction_power
    ...
    env.phm_state.delta_t[env_ids] = delta_t

    current_est = torch.abs(applied_torque) / (KT_NOMINAL * GEAR_RATIO + EPS)
    env.phm_state.real_current[env_ids] = current_est
    ...
    # [1] 순간 전력
    if hasattr(env.phm_state, "instant_power"):
        env.phm_state.instant_power[env_ids] = joint_power_load
    ...
    if hasattr(env.phm_state, "avg_power_log"):
        ...
        env.phm_state.avg_power_log[env_ids] += joint_power_load * ratio

    # 에너지 적분
    total_power_env = torch.sum(joint_power_load, dim=1, keepdim=True)
    step_energy = total_power_env * dt
    env.phm_state.cumulative_energy[env_ids] += step_energy.squeeze(-1)

    # Reward Source
    if hasattr(env.phm_state, "step_energy_log"):
        env.phm_state.step_energy_log[env_ids] += step_energy.squeeze(-1)
    ...
    if hasattr(env.phm_state, "soc"):
        env.phm_state.soc[env_ids] = torch.clamp(
            env.phm_state.soc[env_ids] - soc_drop, 
            min=0.0, max=1.0
        )

    # B. Voltage Write-back
    if hasattr(env.phm_state, "battery_voltage"):
        current_soc = env.phm_state.soc[env_ids]
        ...
        if hasattr(env.phm_state, "battery_voltage_true"):
            env.phm_state.battery_voltage_true[env_ids] = true_voltage

        if hasattr(env.phm_state, "voltage_sensor_bias"):
            sensor_bias = env.phm_state.voltage_sensor_bias[env_ids]
        else:
        ...
        env.phm_state.battery_voltage[env_ids] = torch.clamp(final_voltage, 0.0, 60.0)

        # Optional 8-cell observable channel (for replay governor parity with real logs).
        if hasattr(env.phm_state, "cell_voltage"):
            cell_ocv_bias = getattr(env.phm_state, "cell_ocv_bias", None)
            ...
            cell_ir = getattr(env.phm_state, "cell_internal_resistance", None)
            ...
            cell_sensor_bias = getattr(env.phm_state, "cell_sensor_bias", None)
            ...
            env.phm_state.cell_voltage[env_ids] = torch.clamp(cell_meas, 2.5, 4.25)
    ...
    runtime_limits = robot.data.joint_effort_limits[env_ids][:, env.phm_joint_indices]
    safe_runtime_limits = torch.clamp(runtime_limits, min=1e-6)
    saturation_ratio = torch.abs(applied_torque) / safe_runtime_limits
    if hasattr(env.phm_state, "torque_saturation"):
        env.phm_state.torque_saturation[env_ids] = torch.maximum(
            env.phm_state.torque_saturation[env_ids], saturation_ratio
        )
    ...
    if hasattr(env, "_nominal_effort_limits") and env._nominal_effort_limits is not None:
        nominal_limits = env._nominal_effort_limits[env_ids][:, env.phm_joint_indices]
    else:
    ...
    if hasattr(env.phm_state, "stall_timer"):
        ...
        env.phm_state.stall_timer[env_ids] = torch.where(
            momentary_stall,
            env.phm_state.stall_timer[env_ids] + dt,
            torch.clamp(env.phm_state.stall_timer[env_ids] - STALL_DECAY_RATE * dt, min=0.0)
        )
    ...
    if hasattr(env.phm_state, "temp_derivative"):
        # dt가 너무 작으면 튀니까 보호
        safe_dt = dt if dt > 1e-6 else 1e-6
        temp_rate = (new_temp - current_temp) / safe_dt
        env.phm_state.temp_derivative[env_ids] = temp_rate

    env.phm_state.coil_temp[env_ids] = new_temp
    ...
    if case_quant_step > 0.0 and hasattr(env.phm_state, "motor_case_temp"):
        quant_case = _quantize_channel(env.phm_state.motor_case_temp[env_ids], case_quant_step)
        env.phm_state.motor_case_temp[env_ids] = torch.clamp(quant_case, min=T_AMB, max=200.0)
        if hasattr(env.phm_state, "case_temp"):
            env.phm_state.case_temp[env_ids] = env.phm_state.motor_case_temp[env_ids]
    ...
    env.phm_state.thermal_status[env_ids] = status_vals

    # -------------------------------------------------------------------------
    # 7. 버퍼 및 동기화
    # -------------------------------------------------------------------------
    if hasattr(env, "phm_short_term_buffer"):
        # [Fix] env_ids를 전달하여 해당 환경만 EMA/충격량 갱신.
        # 반환값은 이미 env_ids 범위에 해당하는 크기이므로 재슬라이싱 불필요.
        metrics = env.phm_short_term_buffer.update(env, env_ids=env_ids)
        
        if hasattr(env.phm_state, "jitter_intensity"):
            jitter_data = metrics["jitter"]
            val = torch.maximum(env.phm_state.jitter_intensity[env_ids], jitter_data)
            env.phm_state.jitter_intensity[env_ids] = torch.nan_to_num(val, nan=0.0)

        if hasattr(env.phm_state, "impact_intensity") and metrics["impact_jerk"].numel() > 0:
            impact_data = metrics["impact_jerk"]
            max_impact, _ = torch.max(impact_data, dim=1)
            env.phm_state.impact_intensity[env_ids] = torch.nan_to_num(max_impact, nan=0.0)

    if hasattr(env, "phm_long_term_buffer"):
        # [Fix] env_ids를 전달하여 해당 환경만 열 누적/타이머/스냅샷 갱신.
        env.phm_long_term_buffer.update(env, dt, env_ids=env_ids)

    # env.extras 업데이트
    if "joint_temp" in env.extras:
        env.extras["joint_temp"][env_ids] = env.phm_state.coil_temp[env_ids]
    if "joint_case_temp" in env.extras and hasattr(env.phm_state, "motor_case_temp"):
        env.extras["joint_case_temp"][env_ids] = env.phm_state.motor_case_temp[env_ids]
...
def _curriculum_landmarks(env: ManagerBasedRLEnv) -> tuple[float, float, float, float, float]:
    """
    Curriculum step landmarks for PHM scenario mix.

    Primary path (recommended):
      - iteration milestones from cfg:
        used_start -> used_end -> aged_end -> critical_end -> final_end
      - converted to simulation steps via phm_curriculum_steps_per_iter.
    ...
    if cfg is not None and hasattr(cfg, "phm_curriculum_used_start_iter"):
        try:
            steps_per_iter = max(float(getattr(cfg, "phm_curriculum_steps_per_iter", 24)), 1.0)
            i0 = int(getattr(cfg, "phm_curriculum_used_start_iter", 1000))
            i1 = int(getattr(cfg, "phm_curriculum_used_end_iter", 1400))
            i2 = int(getattr(cfg, "phm_curriculum_aged_end_iter", 1900))
            i3 = int(getattr(cfg, "phm_curriculum_critical_end_iter", 2400))
            i4 = int(getattr(cfg, "phm_curriculum_final_end_iter", 3000))
...
def _curriculum_mix_from_step(
    ...
    if s <= S0:
        ...
        # Final settling:
        # fresh 0.15 -> 0.10, used 0.40 fixed, aged 0.30 fixed, critical 0.15 -> 0.20
        t = _ramp01(s, S3, S4)
        p_fresh = 0.15 + (0.10 - 0.15) * t
        p_used = 0.40
        p_aged = 0.30
        p_crit = 0.15 + (0.20 - 0.15) * t
    else:
...
def _curriculum_effective_step_with_gate(env: ManagerBasedRLEnv, current_step: int, num_resets: int) -> tuple[float, float, float, float]:
    ...
    - Optional: walking-performance gate if cfg.phm_curriculum_use_performance_gate=True.
    ...
    tracking_ema is normalized velocity tracking error:
      ||v_cmd - v|| / (||v_cmd|| + 0.20)
    """
    use_gate = bool(getattr(getattr(env, "cfg", None), "phm_curriculum_use_performance_gate", False))
    if not use_gate:
        if hasattr(env, "extras"):
            env.extras["phm/curriculum_effective_step"] = torch.tensor(float(max(current_step, 0)), device=env.device)
            env.extras["phm/curriculum_term_ema"] = torch.tensor(0.0, device=env.device)
            env.extras["phm/curriculum_track_ema"] = torch.tensor(0.0, device=env.device)
            env.extras["phm/curriculum_advance_gain"] = torch.tensor(1.0, device=env.device)
    ...
    if hasattr(env, "extras"):
        env.extras["phm/curriculum_effective_step"] = torch.tensor(float(env._curriculum_effective_step), device=env.device)
        env.extras["phm/curriculum_term_ema"] = torch.tensor(term_ema, device=env.device)
        env.extras["phm/curriculum_track_ema"] = torch.tensor(track_ema, device=env.device)
        env.extras["phm/curriculum_advance_gain"] = torch.tensor(float(advance_gain), device=env.device)
...
# =============================================================================
def reset_phm_interface(env: ManagerBasedRLEnv, env_ids: torch.Tensor):
    """
    [PHM Reset Logic: Hybrid Curriculum Randomization]
    ...
    # ---------------------------------------------------------------------
    if hasattr(env.phm_state, "degraded_stiffness") and hasattr(env.phm_state, "degraded_damping"):
        joint_idx = getattr(env, "phm_joint_indices", slice(None))
        if (
            ...
            if isinstance(joint_idx, slice):
                env.phm_state.degraded_stiffness[env_ids] = nom_stiff_subset
                env.phm_state.degraded_damping[env_ids] = nom_damp_subset
            else:
                env.phm_state.degraded_stiffness[env_ids] = nom_stiff_subset[:, joint_idx]
                env.phm_state.degraded_damping[env_ids] = nom_damp_subset[:, joint_idx]
        else:
            raise RuntimeError(
                "[PHM Error] '_nominal_stiffness' not found in Env. "
                "Ensure actutators are configured correctly in EnvCfg."
            )

    # 버퍼 초기화
    if hasattr(env, "phm_long_term_buffer"):
        env.phm_long_term_buffer.reset(env_ids)
    if hasattr(env, "phm_short_term_buffer"):
        env.phm_short_term_buffer.reset(env_ids)
    ...
    env.phm_state.friction_bias[env_ids] = friction_noise
    ...
    env.phm_state.voltage_sensor_bias[env_ids] = sensor_noise
    ...
    if hasattr(env.phm_state, "cell_ocv_bias"):
        lo, hi = float(cell_bias_range[0]), float(cell_bias_range[1])
        if hi < lo:
            lo, hi = hi, lo
        env.phm_state.cell_ocv_bias[env_ids] = torch.rand((num_resets, 8), device=device) * (hi - lo) + lo

    if hasattr(env.phm_state, "cell_internal_resistance"):
        lo, hi = float(cell_ir_range[0]), float(cell_ir_range[1])
        if hi < lo:
            lo, hi = hi, lo
        env.phm_state.cell_internal_resistance[env_ids] = torch.rand((num_resets, 8), device=device) * (hi - lo) + lo

    if hasattr(env.phm_state, "cell_sensor_bias"):
        lo, hi = float(cell_sensor_bias_range[0]), float(cell_sensor_bias_range[1])
        if hi < lo:
            lo, hi = hi, lo
        env.phm_state.cell_sensor_bias[env_ids] = torch.rand((num_resets, 8), device=device) * (hi - lo) + lo

    # [Fix] Encoder offset randomization (was always zero — dead feature)
    # Simulates per-robot fixed calibration error (~0.005 rad ≈ 0.3°)
    if hasattr(env.phm_state, "encoder_offset"):
        env.phm_state.encoder_offset[env_ids] = (
            torch.rand((num_resets, NUM_MOTORS), device=device) - 0.5
        ) * 0.01
    
    if hasattr(env.phm_state, "min_voltage_log"):
         env.phm_state.min_voltage_log[env_ids] = 33.6
    if hasattr(env.phm_state, "brownout_latched"):
         env.phm_state.brownout_latched[env_ids] = False
    ...
    forced_scenario = str(getattr(cfg_obj, "phm_force_scenario_label", "none")).strip().lower()
    if forced_scenario in {"", "none", "off"}:
        forced_scenario = "none"
    if forced_scenario not in {"none", "fresh", "used", "aged", "critical"}:
        if not _PHM_LOG_FLAGS["forced_scenario_invalid_warned"]:
            logging.warning(
                "[PHM] Invalid phm_force_scenario_label='%s'. Ignoring forced scenario.",
                forced_scenario,
            )
            _PHM_LOG_FLAGS["forced_scenario_invalid_warned"] = True
        forced_scenario = "none"
    if forced_scenario != "none":
        ...
        if not _PHM_LOG_FLAGS["forced_scenario_info_logged"]:
            logging.info("[PHM] Forced reset scenario active: %s", forced_scenario)
            _PHM_LOG_FLAGS["forced_scenario_info_logged"] = True
    ...
    if hasattr(env, "extras"):
        env.extras["phm/curriculum_p_fresh"] = torch.tensor(p_fresh, device=device)
        env.extras["phm/curriculum_p_used"] = torch.tensor(p_used, device=device)
        env.extras["phm/curriculum_p_aged"] = torch.tensor(p_aged, device=device)
        env.extras["phm/curriculum_p_crit"] = torch.tensor(p_crit, device=device)
    ...
    fault_mode = str(getattr(cfg_obj, "phm_fault_injection_mode", "single_motor_random")).strip().lower()
    if fault_mode not in {"all_motors_random", "single_motor_random", "single_motor_fixed"}:
        if not _PHM_LOG_FLAGS["fault_mode_invalid_warned"]:
            logging.warning(
                "[PHM] Invalid phm_fault_injection_mode='%s'. Fallback to single_motor_random.",
                fault_mode,
            )
            _PHM_LOG_FLAGS["fault_mode_invalid_warned"] = True
        fault_mode = "single_motor_random"
    fixed_motor_id = int(getattr(cfg_obj, "phm_fault_fixed_motor_id", -1))
    if fault_mode == "single_motor_fixed" and not (0 <= fixed_motor_id < NUM_MOTORS):
        if not _PHM_LOG_FLAGS["fault_fixed_id_invalid_warned"]:
            logging.warning(
                "[PHM] phm_fault_fixed_motor_id=%s is invalid for mode single_motor_fixed. "
    ...
    def _set_case_from_coil(ids: torch.Tensor, delta_low: float, delta_high: float):
        ...
        case_temp = torch.clamp(env.phm_state.coil_temp[ids] - delta, min=T_AMB)
    ...
    if torch.any(mask_used):
        ...
        env.phm_state.fatigue_index[ids] = used_fatigue
        env.phm_state.motor_health_capacity[ids] = used_health
        env.phm_state.coil_temp[ids] = used_coil
        _set_case_from_coil(ids, 1.0, 3.5)
        if hasattr(env.phm_state, "fault_mask"):
            env.phm_state.fault_mask[ids] = used_fault_mask
        if hasattr(env.phm_state, "fault_motor_id"):
            env.phm_state.fault_motor_id[ids] = used_fault_motor
        if hasattr(env.phm_state, "soc"):
            env.phm_state.soc[ids] = torch.rand(len(ids), device=device) * 0.2 + 0.8
    ...
    if torch.any(mask_aged):
        ...
        env.phm_state.fatigue_index[ids] = aged_fatigue
        env.phm_state.motor_health_capacity[ids] = aged_health
        env.phm_state.coil_temp[ids] = aged_coil
        _set_case_from_coil(ids, 2.5, 6.5)
        if hasattr(env.phm_state, "fault_mask"):
            env.phm_state.fault_mask[ids] = aged_fault_mask
        if hasattr(env.phm_state, "fault_motor_id"):
            env.phm_state.fault_motor_id[ids] = aged_fault_motor
        if hasattr(env.phm_state, "soc"):
            env.phm_state.soc[ids] = torch.rand(len(ids), device=device) * 0.4 + 0.4
    ...
    try:
        ...
        if not _PHM_LOG_FLAGS["thermal_reset_params_read_failed"]:
            logging.warning(
                "[PHM] Failed to read thermal reset params for critical scenario adaptation; "
                "using default critical coil range. error=%s",
                err,
            )
            _PHM_LOG_FLAGS["thermal_reset_params_read_failed"] = True
    ...
    if torch.any(mask_crit):
        ...
        env.phm_state.fatigue_index[ids] = crit_fatigue
        env.phm_state.motor_health_capacity[ids] = crit_health
        env.phm_state.coil_temp[ids] = crit_coil
        _set_case_from_coil(ids, 3.5, 8.0)
        if hasattr(env.phm_state, "fault_mask"):
            env.phm_state.fault_mask[ids] = crit_fault_mask
        if hasattr(env.phm_state, "fault_motor_id"):
            env.phm_state.fault_motor_id[ids] = crit_fault_motor
        if hasattr(env.phm_state, "soc"):
            env.phm_state.soc[ids] = torch.rand(len(ids), device=device) * 0.2 + 0.1

    if hasattr(env.phm_state, "temp_derivative"):
        env.phm_state.temp_derivative[env_ids] = 0.0
    ...
    if case_quant_step > 0.0 and hasattr(env.phm_state, "motor_case_temp"):
        ...
        if hasattr(env.phm_state, "case_temp"):
            env.phm_state.case_temp[env_ids] = env.phm_state.motor_case_temp[env_ids]
    if hasattr(env.phm_state, "encoder_hold_flag"):
        env.phm_state.encoder_hold_flag[env_ids] = 0.0
    ...
    if hasattr(env.phm_state, "soc"):
        pack_v_reset = compute_battery_voltage(
            env.phm_state.soc[env_ids],
            torch.zeros((num_resets,), device=device, dtype=torch.float32),
        )
        if hasattr(env.phm_state, "battery_voltage_true"):
            env.phm_state.battery_voltage_true[env_ids] = pack_v_reset
        sensed_pack_v = pack_v_reset + env.phm_state.voltage_sensor_bias[env_ids]
        v_quant_step = float(getattr(getattr(env, "cfg", None), "battery_voltage_quant_step_v", 0.0))
        sensed_pack_v = _quantize_channel(sensed_pack_v, v_quant_step)
        env.phm_state.battery_voltage[env_ids] = torch.clamp(sensed_pack_v, 0.0, 60.0)
        if hasattr(env.phm_state, "min_voltage_log"):
            env.phm_state.min_voltage_log[env_ids] = env.phm_state.battery_voltage_true[env_ids]

        if hasattr(env.phm_state, "cell_voltage"):
            cell_base = pack_v_reset.unsqueeze(-1) / 8.0 + env.phm_state.cell_ocv_bias[env_ids]
            cell_meas = cell_base + env.phm_state.cell_sensor_bias[env_ids]
    ...
    if friction_source is not None:
        try:
            if isinstance(joint_indices, slice):
                env.phm_state.base_friction_torque[env_ids] = friction_source[env_ids]
            else:
                env.phm_state.base_friction_torque[env_ids] = friction_source[env_ids][:, joint_indices]
        except Exception as err:
            if not _PHM_LOG_FLAGS["friction_source_reset_failed"]:
                logging.warning(
                    "[PHM] Failed to sync reset joint friction coefficients; "
                    "keeping nominal defaults. error=%s",
                    err,
                )
                _PHM_LOG_FLAGS["friction_source_reset_failed"] = True
    ...
    if hasattr(env, "phm_long_term_buffer"):
        ...
        init_fatigue = env.phm_state.fatigue_index[env_ids]  # (M, J)
        env.phm_long_term_buffer.fatigue_snapshots[env_ids, 0, :] = init_fatigue
        env.phm_long_term_buffer.snapshot_index[env_ids] = 1
        env.phm_long_term_buffer.fill_count[env_ids] = 1
