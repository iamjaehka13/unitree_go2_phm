# unitree_go2_phm/phm/buffers/short_term.py
...
class ShortTermHealthBuffer:
    """
    [PHM Core] 고주파 물리 과도 현상(Transient Physics) 추적 버퍼 (Safety Patched v3.2).
    Safe Features:
    ...
    def __init__(
        ...
        if target_sensor_name == "0":
            logging.warning("[PHM Warning] Contact sensor name is '0'. Forcing 'contact_forces'.")
        ...
        if self.contact_sensor is None:
            logging.warning("[PHM Warning] Contact sensor not found during init. Impact analysis will be empty until resolved.")
        else:
            if getattr(self.contact_sensor, "num_bodies", 0) == 0:
                logging.warning("[PHM Warning] Contact sensor is not tracking any bodies. Impact analysis will be empty.")
        # 2. 조인트 인덱스 저장
