...
from .mdp.actuators import PhmRealismActuator
...
# -------------------------------------------------------------------------
@configclass
class UnitreeGo2PhmSceneCfg(InteractiveSceneCfg):
    """Configuration for the Unitree Go2 PHM scene."""
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        # Reference locomotion observation core + PHM channels
        ...
        # joint_pos/joint_vel already include PHM sensor corruption in phm_mdp.*_phm.
        ...
        energy_budget = ObsTerm(func=phm_mdp.available_voltage_budget, params={"cutoff_voltage": 24.5}, noise=None)
        thermal_stress = ObsTerm(func=phm_mdp.get_thermal_stress_index, params={"asset_cfg": SceneEntityCfg("robot")}, noise=GaussianNoiseCfg(std=0.015))
        mech_health = ObsTerm(func=phm_mdp.get_mechanical_health_index, params={"asset_cfg": SceneEntityCfg("robot")})
        
        vibration_level = ObsTerm(
            func=phm_mdp.weighted_contact_acceleration,
            params={
    ...
    class CriticCfg(ObsGroup):
        # Reference critic core + PHM channels. No corruption for value stability.
        ...
        last_action = ObsTerm(func=mdp.last_action, clip=(-100, 100))
        energy_budget = ObsTerm(func=phm_mdp.available_voltage_budget, params={"cutoff_voltage": 24.5}, noise=None)
        thermal_stress = ObsTerm(func=phm_mdp.get_thermal_stress_index, params={"asset_cfg": SceneEntityCfg("robot")}, noise=None)
        mech_health = ObsTerm(func=phm_mdp.get_mechanical_health_index, params={"asset_cfg": SceneEntityCfg("robot")})
        vibration_level = ObsTerm(
            func=phm_mdp.weighted_contact_acceleration,
            params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*_foot"), "imu_cfg": SceneEntityCfg("base_imu")},
        )
        degradation_trend = ObsTerm(func=phm_mdp.degradation_slope, params={"asset_cfg": SceneEntityCfg("robot")})
        thermal_overload = ObsTerm(
            func=phm_mdp.thermal_overload_duration_obs,
            params={"asset_cfg": SceneEntityCfg("robot"), "scale": 0.01},
        )
...
@configclass
class RewardsCfg:
    """Reference locomotion rewards + PHM rewards."""
    ...
    # -------------------------------------------------------------------------
    # 3. PHM Objectives (Health & Efficiency)
    # -------------------------------------------------------------------------
    energy_efficiency = RewTerm(
        func=phm_mdp.electrical_energy_reward, 
        weight=0.01, 
    ...
    bearing_health = RewTerm(
        func=phm_mdp.bearing_life_reward, 
        weight=0.05, 
    ...
    saturation_prevention = RewTerm(
        func=phm_mdp.actuator_saturation_reward,
        weight=0.01,
        params={"std": 0.1}
    )
...
# -------------------------------------------------------------------------
@configclass
class UnitreeGo2PhmEnvCfg(ManagerBasedRLEnvCfg):
    ...
    # - 1601~2400: PHM medium-strength ramp
    # - 2401~3000: hardest PHM mix + stabilization
    ...
    # - "single_motor_fixed"  (deterministic eval; use `phm_fault_fixed_motor_id`)
