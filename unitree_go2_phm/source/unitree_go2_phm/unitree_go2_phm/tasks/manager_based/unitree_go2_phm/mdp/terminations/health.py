# =============================================================================
# unitree_go2_phm/mdp/terminations/health.py
# =============================================================================
...
def motor_stall(
    ...
    """
    [Critical PHM] 모터 구속(Stall) 지속 감지 (Judge Only).
    
    Note:
        이 함수는 물리적인 스톨 판정(속도 < 0.1 rad/s, 토크 > 90% 등)을 수행하지 않습니다.
        물리적 판정은 'phm/interface.py'에서 매 스텝 수행되어 
        'env.phm_state.stall_timer'에 누적됩니다.
        
        여기서는 오직 '누적된 시간'이 허용치(stall_time_threshold_s)를 
        초과했는지만 심판합니다.
        
        * 스톨의 물리적 기준 변경: phm/constants.py -> MIN_STALL_VELOCITY 수정
    """
    # 1. 안전 장치: PHM State가 초기화되지 않았으면 False 반환
...
def thermal_runaway(
    ...
    """
    if not hasattr(env, "phm_state") or not hasattr(env.phm_state, "coil_temp"):
        return torch.zeros(env.num_envs, dtype=torch.bool, device=env.device)
    ...
    if use_case_proxy:
        ...
        for name in (
            ...
            if hasattr(env.phm_state, name):
                val = getattr(env.phm_state, name)
                if isinstance(val, torch.Tensor):
                    case_like = val
                    break
        if case_like is None:
            case_like = env.phm_state.coil_temp - float(coil_to_case_delta_c)
        temps = case_like
...
def energy_depletion(
    ...
    current_energy = env.phm_state.cumulative_energy
